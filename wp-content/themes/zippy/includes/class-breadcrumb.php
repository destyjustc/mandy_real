...
if(!class_exists('zippy_breadcrumb'))
{

	class zippy_breadcrumb
	{
		var $options;
		
	function zippy_breadcrumb($options = ""){
		...
		function zippy_simple_breadcrumb_case($der_post)
		{
			...
			if (is_page()){
				 if($der_post->post_parent) {
					 ...
					 $this->zippy_simple_breadcrumb_case($my_query);
					 $link = '<a href="';
		...
		if(is_404()){ return _e("404 - Page not Found",'zippy_framework'); }
		
		if(is_search()){ return _e("Search",'zippy_framework');}	
...
/*-----------------------------------------------------------------------------------*/
/* zippy_breadcrumbs() - Custom breadcrumb generator function  */
/*
/* Params:
/*
/* Arguments Array:
...
function zippy_breadcrumbs( $args = array() ) {
	...
	$defaults = array(
		'separator' => '&raquo;',
		'before' => '<span class="breadcrumb-title">' . __( 'You are here:', 'zippy') . '</span>',
		'after' => false,
		'front_page' => true,
		'show_home' => __( 'Home', 'zippy'),
		'echo' => false, 
	...
	$args = apply_filters( 'zippy_breadcrumbs_args', $args );
	...
	elseif ( is_home() ) {
		...
		$trail = array_merge( $trail, zippy_breadcrumbs_get_parents( $home_page->post_parent, '' ) );
	...
	elseif ( is_singular() ) {
		...
		if ( 'page' !== $post_type && 'post' !== $post_type ) {
			...
			if ( !empty( $path ) )
				$trail = array_merge( $trail, zippy_breadcrumbs_get_parents( '', $path ) );
		...
		*/
		...
		if ( empty( $path ) && 0 !== $parent || 'attachment' == $post_type )
			$trail = array_merge( $trail, zippy_breadcrumbs_get_parents( $parent, '' ) );
		...
		if ( 'post' == $post_type && $show_posts_page == true && 'page' == get_option( 'show_on_front' ) ) {
			...
			if ( $posts_page != '' && is_numeric( $posts_page ) ) {
				$trail = array_merge( $trail, zippy_breadcrumbs_get_parents( $posts_page, '' ) );
			}
		}
	...
	elseif ( is_archive() ) {
		...
		if ( is_tax() || is_category() || is_tag() ) {
			...
			if ( is_taxonomy_hierarchical( $term->taxonomy ) && $term->parent )
				$trail = array_merge( $trail, zippy_breadcrumbs_get_term_parents( $term->parent, $term->taxonomy ) );
		...
		elseif ( is_time() ) {
			...
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g:i a', 'zippy') );

			elseif ( get_query_var( 'minute' ) )
				$trail['trail_end'] = sprintf( __( 'Minute %1$s', 'zippy'), get_the_time( __( 'i', 'zippy') ) );

			elseif ( get_query_var( 'hour' ) )
				$trail['trail_end'] = get_the_time( __( 'g a', 'zippy') );
		}
		...
		elseif ( is_date() ) {
			...
			if ( $wp_rewrite->front )
				$trail = array_merge( $trail, zippy_breadcrumbs_get_parents( '', $wp_rewrite->front ) );

			if ( is_day() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'zippy') ) . '">' . get_the_time( __( 'Y', 'zippy') ) . '</a>';
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'zippy') ) . '">' . get_the_time( __( 'F', 'zippy') ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'j', 'zippy') );
			}

			elseif ( get_query_var( 'w' ) ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'zippy') ) . '">' . get_the_time( __( 'Y', 'zippy') ) . '</a>';
				$trail['trail_end'] = sprintf( __( 'Week %1$s', 'zippy'), get_the_time( esc_attr__( 'W', 'zippy') ) );
			}

			elseif ( is_month() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'zippy') ) . '">' . get_the_time( __( 'Y', 'zippy') ) . '</a>';
				$trail['trail_end'] = get_the_time( __( 'F', 'zippy') );
			}

			elseif ( is_year() ) {
				$trail['trail_end'] = get_the_time( __( 'Y', 'zippy') );
			}
		}
	}
	...
	elseif ( is_search() )
		$trail['trail_end'] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'zippy'), esc_attr( get_search_query() ) );

	/* If viewing a 404 error page. */
	elseif ( is_404() )
		$trail['trail_end'] = __( '404 Not Found', 'zippy');

	/* Allow child themes/plugins to filter the trail array. */
	$trail = apply_filters( 'zippy_breadcrumbs_trail', $trail, $args );
	...
	$breadcrumb = apply_filters( 'zippy_breadcrumbs', $breadcrumb );
...
} // End zippy_breadcrumbs()


function zippy_breadcrumbs_get_parents( $post_id = '', $path = '' ) {
...
} // End zippy_breadcrumbs_get_parents()


function zippy_breadcrumbs_get_term_parents( $parent_id = '', $taxonomy = '' ) {
...
} // End zippy_breadcrumbs_get_term_parents()