...
class Options_Framework_Admin {
    ...
    function options_notice() {
        ...
        if ( !is_multisite() && ( $pagenow == 'plugins.php' || $pagenow == 'themes.php' ) ) {
			...
			if ( ! get_user_meta($user_id, 'optionsframework_ignore_notice') ) {
				...
				printf( __('Your current theme does not have support for the Options Framework plugin.  <a href="%1$s" target="_blank">Learn More</a> | <a href="%2$s">Hide Notice</a>', 'optionsframework'), esc_url('http://wptheming.com/options-framework-plugin'), '?optionsframework_nag_ignore=0');
				echo "</p></div>";
			}
        }
	}
    ...
    function settings_init() {
		...
		register_setting( ZIPPY_OPTIONS_PREFIXED.'optionsframework', $optionsframework_settings['id'],  array ( $this, 'validate_options' ) );
...
function add_my_options_page() {
	...
	function enqueue_admin_styles() {
	    global $pagenow;
		if($pagenow == "themes.php" && isset($_GET['page']) && $_GET['page']=="zippy-options"){
		wp_enqueue_style( 'optionsframework', ZIPPY_OPTIONS_FRAMEWORK_URI.'css/optionsframework.css', array(),  Options_Framework::VERSION );
	...
	function enqueue_admin_scripts( $hook ) {
     global $pagenow;
       ...
       if($pagenow == "themes.php" && isset($_GET['page']) && $_GET['page']=="zippy-options"){
		...
		wp_enqueue_script( 'options-custom', ZIPPY_OPTIONS_FRAMEWORK_URI . 'js/options-custom.js', array( 'jquery','wp-color-picker' ), Options_Framework::VERSION );
          }
	 ...
	 function options_page() { ?>
	    ...
	    <div id="optionsframework-metabox" class="metabox-holder">
		    <div id="optionsframework" class="postbox">
				...
				<?php settings_fields( ZIPPY_OPTIONS_PREFIXED.'optionsframework' ); ?>
